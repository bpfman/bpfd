...
use crate::{
    ...
    types::{
        BpfProgType, BytecodeImage, Direction, ListFilter,
        ProbeType::{self, *},
        Program, ProgramData, LINKS_LINK_PREFIX, PROGRAM_PREFIX,
    },
    utils::{
...
///     let kprobe_program = KprobeProgram::new(program_data, is_retprobe)?;
///
///     // Add the kprobe program using the bpfman manager.
///     let added_program = add_programs(vec![Program::Kprobe(kprobe_program)])?;
///
...
fn add_programs_internal(mut programs: Vec<Program>) -> Result<Vec<Program>, BpfmanError> {
    ...
    for (program, id) in programs.iter_mut().zip(program_ids) {
        ...
        save_map(root_db, program, program.get_data().get_id()?, map_owner_id)?;
    }
